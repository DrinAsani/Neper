...
## Player Sprite Sheet (Tulla)
Place your player sprite sheet at:
- `assets/sprites/player/tulla_sprite.png`

Expected grid format:
- 4 columns x 4 rows (16 frames total)
- Rows:
  1. facing down
  2. facing left
  3. facing right
...
Behavior:
- If not moving, the idle frame (column 1) is used.
- If moving, walk frames (columns 2-4) animate.
- Direction row is chosen from player movement direction.
- If the sprite sheet is missing or fails to load, NEPER automatically falls back to the rectangle player.
